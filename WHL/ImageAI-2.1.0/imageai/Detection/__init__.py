        ...
        if (input_file_path == "" and camera_input == None):
            ...
            try:
                if (self.__modelType == "retinanet"):
                    ...
                    video_frames_count = 0
                    # New code
                    cnn_face_detector = dlib.cnn_face_detection_model_v1("/content/ImageAI/mmod_human_face_detector.dat")
                    number_of_CNN_pases = 1
                    # ---------
                    while (input_video.isOpened()):
                        ret, frame = input_video.read()
                        #New code
                        ...
                        if (ret == True):
                            ...
                            #New code on counting people

                            output_frames_dict[counting] = output_objects_array
                            caption = "{} {}".format("Total people in frame: ", str(counting_people))
                            print(caption)
                            cv2.putText(detected_copy, caption, (40,60), cv2.FONT_HERSHEY_PLAIN, 2, (255, 100, 0), 2)
                            #-----
                            #New code for facial detection

                            # apply face detection (cnn)
                            faces_cnn = cnn_face_detector(clean_image, number_of_CNN_pases)
                            ...
                            for face in faces_cnn:
                                ...
                                # draw box over face
                                cv2.rectangle(detected_copy, (x,y), (x+w,y+h), (0,0,255), 2)
                                counting_faces = counting_faces +1
                            caption = "{} {}".format("Total faces in frame: ", str(counting_faces))
                            print(caption)
                            cv2.putText(detected_copy, caption, (40,100), cv2.FONT_HERSHEY_PLAIN, 2, (100, 255, 100), 2)
